...
from http_security_test.cli import outcome, run
...
def test_a_refused_hop_records_why():
    ...
    hop = run.analysed(_facts(hops=hops), REPORT)["source"]["hops"][0]
...
def test_hops_are_present_even_when_empty():
    ...
    assert run.analysed(_facts(), REPORT)["source"]["hops"] == []


def test_a_failed_result_carries_the_kind_and_no_report():
    bad = outcome.Failure("down.example", "dns", "Name or service not known")
...
def test_failures_and_successes_share_one_results_list():
    document = run.run_document(
        [
            run.analysed(_facts(), REPORT),
            run.failed(outcome.Failure("down.example", "dns", "nope")),
        ],
