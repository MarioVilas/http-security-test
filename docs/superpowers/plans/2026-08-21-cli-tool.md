...
- **A module and an exported callable must not share a name.** `from .live import fetch` would set `http_security_test.cli.live` to the function and
...
- Produces: `cli.main(argv=None) -> int`; `cli.meta.TOOL_NAME` (str),
  ...
  `cli.options.VERBS` (tuple), `cli.options.build_parser() -> argparse.ArgumentParser`, `cli.options.misplaced_target(argv) -> str or None`;
...
- Produces: `Hop(origin, code, destination, followed=True, refused=None)`;
  `Exchange(kind, target, url, status, reason, headers, hops=(), raw_response=None, raw_request=None)`; `Failure(target, kind, message)`;
...
- Produces: `matches(pattern, hostname) -> bool`; `allows(patterns, hostname) -> bool`; `derive(hosts) -> tuple`; `resolve(explicit, target_hosts) -> tuple`;
...
- Produces: `Options(method, headers, user_agent, timeout, insecure, proxy, no_redirect, max_redirects, patterns, raw)` namedtuple; `classify(error) -> str or None`; `_Chain(patterns, follow=True, limit=10)`; `build_opener(options) -> (opener, chain)`; `raw_head(response) -> str`; `raw_request(url, options) -> str`; `fetch(target, options, opener=None, chain=None) -> tuple`.
