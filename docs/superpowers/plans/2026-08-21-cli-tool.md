...
@pytest.mark.parametrize("spec", ["-", "out", "out.csv", "json:", "nope:out.json"])
...
CLASSIFY_CASES = [
    ...
    (urllib.error.URLError(http.client.BadStatusLine("garbage")), "protocol"),
...
def fetch(target, options, opener=None, chain=None):
    ...
    try:
        ...
        pass  # the headers are what we came for and we already have them
    finally:
        response.close()
...
def test_min_level_filters_the_terminal_but_not_the_file(tmp_path, capsys):
    ...
    commands.do_scan(
        parse(
            ["scan", "--min-level", "error", "-o", "json:%s" % path, "https://a.test/"]
        ),
    ...
    # The evidence file is complete; the terminal is filtered.
